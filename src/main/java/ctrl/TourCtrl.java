...
public class TourCtrl {
    ...
    // Tạo tour mới
    public void createTour(String ID,String description, String departurePoint, double pricePerPerson, int maxPeople, String startDate, String endDate) throws InvalidDateException, DuplicateException, IllegalStateException {
        for (Tour tour: tours){
        ...
        if (pricePerPerson < 0){
            throw new IllegalStateException("Giá phải lớn hơn 0");
        }
        if (maxPeople < 0){
            throw new IllegalStateException("Số lượng người phải lớn hơn 0");
        }
    ...
    public void updateTourInfo(String ID, String description, String departurePoint, double pricePerPerson, int maxPeople, String startDate, String endDate, int bookedPeople) throws InvalidDateException, TourAlreadyBookedException {
        ...
        for (Tour tour: tours){
            if ((tour.getTourID()).equals(ID)){
                // xem có ai booking tour này chưa nếu có thì báo lỗi
                if ((tour.bookedPeople)>0){
